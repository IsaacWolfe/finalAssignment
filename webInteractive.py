...
def webInteractive(subjectP, messageBodyP, nameEmail):
    ...
    # Wait and tab to compose button
        ...
        emailInsert.perform()
        time.sleep(1)
        emailSend = ActionChains(browser)
        emailSend.send_keys(Keys.ENTER, Keys.TAB)
        emailSend.perform()
        time.sleep(2)

        subjectInsert = ActionChains(browser)
        subjectInsert.send_keys(subject)
        subjectInsert.perform()
        time.sleep(1)
        subjectSend = ActionChains(browser)
        subjectSend.send_keys(Keys.TAB)
        subjectSend.perform()
        time.sleep(2)

        bodyInsert = ActionChains(browser)
        bodyInsert.send_keys(messageBody)
        bodyInsert.perform()
        time.sleep(1)
        bodySend = ActionChains(browser)
        bodySend.send_keys(Keys.TAB,Keys.ENTER)
        bodySend.perform()
        time.sleep(2)
